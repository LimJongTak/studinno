...
function adminChartCard() {
  ...
  <div class="card">
    ...
    <div class="vbars">
      ${data
        .map((d) => `<div class="vbar"><div class="vbar-track"><div class="vbar-fill" style="height:${Math.round((d.v / max) * 100)}%"><span class="vbar-val">${d.v}</span></div></div><span class="vbar-lb">${d.m}</span></div>`)
        .join("")}
    </div>
    <div class="kpi-row">
...
function adminMonitorCard() {
  ...
  <div class="card">
    ...
    <div class="monitor-legend">
      ...
      ${running
        .map((e) => `<span class="muted sm">${e.name}: CPU ${e.cpu}% · RAM ${e.ram}%</span>`)
        .join("")}
    </div>
  </div>`;
}
function filteredMembers() {
    ...
    return DATA.members.filter((m) => {
        if (memberRole !== "all" && m.role !== memberRole)
            return false;
        if (!q)
            return true;
...
function memberRows() {
    ...
    if (!list.length)
        return emptyState("조건에 맞는 회원이 없습니다.", "👤");
    return `<table class="tbl"><thead><tr><th>ID</th><th>이름</th><th>구분</th><th>성적</th><th>상태</th></tr></thead><tbody>
    ${list
        .map((m) => `<tr><td>${m.id}</td><td>${m.name}</td><td>${m.role}</td><td><input class="grade-input" value="${m.grade}"/></td><td><span class="badge ${m.status === "활성" ? "done" : "open"}">${m.status}</span></td></tr>`)
        .join("")}
  </tbody></table>`;
}
function adminMembersCard() {
  ...
  <div class="card">
    ...
    <div class="cat-tabs" id="memberRoleTabs">${roles
        .map((r) => `<button class="cat-tab ${memberRole === r ? "active" : ""}" data-mr="${r}">${r === "all" ? "전체" : r}</button>`)
        .join("")}</div>
...
function refreshMemberTable() {
    ...
    if (wrap)
        wrap.innerHTML = memberRows();
}
function adminProjectAdminCard() {
    return `
  <div class="card">
    ...
    <div class="adm-proj-list">
      ${DATA.projects
        ...
        return `<div class="adm-proj-row ${hidden ? "is-hidden" : ""}">
          ...
          <button class="btn outline sm" onclick="forceVisibility('${p.id}')">${p.visibility === "public" ? "🌐 공개" : "🔒 비공개"}</button>
          <button class="btn outline sm" onclick="toggleHideProject('${p.id}')">${hidden ? "🙈 숨김됨" : "👁 표시"}</button>
        </div>`;
    })
        .join("")}
    </div>
...
function forceVisibility(id) {
    ...
    toast(`🔧 <b>${p.title}</b> 공개범위를 ${p.visibility === "public" ? "공개" : "비공개"}로 변경했습니다.`, "info");
...
function toggleHideProject(id) {
    ...
    if (hiddenProjects.has(id))
        ...
        DATA.adminLogs.unshift({
            time: nowHM(),
            type: "숨김",
            msg: `${(p === null || p === void 0 ? void 0 : p.title) || ""} 부적절 프로젝트 숨김(목록 비노출)`,
        });
    }
    toast(hiddenProjects.has(id)
        ? `🙈 <b>${(p === null || p === void 0 ? void 0 : p.title) || ""}</b> 숨김 처리 — 산학프로젝트 목록에서 비노출`
        : `👁 <b>${(p === null || p === void 0 ? void 0 : p.title) || ""}</b> 다시 표시`, "info");
    if (currentPage === "admin")
        render("admin");
}
function adminBadgeCard() {
    return `
  <div class="card">
    ...
    <div class="badge-admin-list">
      ${DATA.badges
        .map((b) => `<div class="badge-admin-row"><span class="bar-ic">${b.icon}</span><div class="bar-main"><b>${b.name}</b><span class="muted sm">${b.type} · ${b.date}</span></div><button class="btn outline sm" onclick="revokeBadge('${b.name.replace(/'/g, "")}')">회수</button></div>`)
        .join("")}
    </div>
  </div>`;
}
function addBadgeTemplate(btn) {
...
function adminSettingsCard() {
    const tog = (k, label) => `<label class="set-row"><span>${label}</span><span class="switch"><input type="checkbox" ${adminSettings[k] ? "checked" : ""} onchange="toggleSetting('${k}')"/><span class="switch-track"></span></span></label>`;
    return `
  <div class="card">
