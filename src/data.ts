...
/* ===== 타입 정의 ===== */
type FilePerm = "public" | "private" | "request";
type ProjectStatus = "진행중" | "모집중" | "완료";
...
type Provider = "gitlab" | "kaggle" | "jupyter";
...
interface User {
  ...
  avatar: string;
  servers: string[];
  linked: { gitlab: boolean; kaggle: boolean; sems: boolean };
}
...
interface Badge {
  ...
  type: string;
  issuer?: string; // 발급기관
  criteria?: string; // 부여 기준
  score?: number; // 정량 평가 결과
  desc?: string; // 설명
}
...
interface Project {
  ...
  files: ProjectFile[];
  ide: "gitlab" | "kaggle";
  license: string; // 오픈소스 라이선스 (MIT / Apache-2.0 ...)
  visibility: "public" | "private"; // 저장소 공개 범위
  req: Record<string, number>; // F-AI 매칭: RFP 요구역량 (skills 6축 라벨 → 목표치)
  reqTags: string[]; // F-AI 매칭: RFP 요구 기술 태그 (#비전 ...)
}
...
interface IdeEnv {
  id: string;
  name: string;
  stack: string; // Python / Node.js / R ...
  version: string;
  provider: Provider; // gitlab(웹IDE) | kaggle | jupyter
...
/* ===== F-ID: GitLab 저장소 관리 (브랜치·MR·이슈·파이프라인) ===== */
interface Branch {
  name: string;
  project: string;
  isDefault: boolean;
...
type MrStatus = "open" | "merged" | "draft" | "closed";
interface MergeRequest {
  ...
  added: number;
  removed: number;
  pipeline: "passed" | "failed" | "running" | "none";
  time: string;
  task?: string; // 연결된 칸반 태스크 id (병합 시 자동 완료)
}
...
interface Issue {
  ...
  project: string;
  title: string;
  label: string; // bug / feature / docs
  assignee: string;
  status: IssueStatus;
  comments: number;
  time: string;
}
type StageStatus = "passed" | "failed" | "running" | "skipped";
interface Pipeline {
  id: string;
  project: string;
  branch: string;
...
/* ===== F-ID(Kaggle): 경진대회·해커톤 대회 서비스 ===== */
type CompType = "경진대회" | "해커톤";
type CompStatus = "진행중" | "예정" | "종료";
interface LeaderRow {
  rank: number;
  team: string;
  score: number;
  entries: number;
  change: number; // 순위 변동(+상승 / -하락 / 0유지)
  isMe: boolean;
  members: number; // 팀 인원
}
type CompPhase = "모집" | "진행" | "채점" | "발표";
interface CompTeam {
  name: string;
  members: string[]; // 팀원 이름(첫번째=팀장=나)
}
interface Competition {
  ...
  type: CompType;
  status: CompStatus;
  metric: string; // 평가지표 (F1 / Accuracy / RMSE ...)
  startDate: string;
  deadline: string;
  prize: string;
  ...
  participants: number;
  joined: boolean;
  myTeam?: CompTeam; // 팀 참가 시 내 팀
  teamMin: number; // 최소 팀 인원
  teamMax: number; // 최대 팀 인원
  dailyLimit: number; // 일일 제출 제한
  usedToday: number; // 오늘 사용한 제출 수
  publicSplit: string; // 예) "Public 30% / Private 70%"
  phase: CompPhase;
  rules: string[];
  tags: string[];
  desc: string;
...
interface Submission {
  id: string;
  comp: string;
  team: string; // 제출 주체(팀명 또는 개인)
  file: string;
  score: number;
  rank: number;
  time: string;
  status: "scored" | "scoring" | "error";
}
...
interface JobTarget {
  id: string;
  name: string;
  target: Record<string, number>; // 스킬 라벨 → 목표치
}
interface LearnRec {
  title: string;
  kind: "교과" | "비교과";
  skill: string; // 보완 대상 역량
  gain: number; // 예상 향상치
}
...
interface Talent {
  ...
  tags: string[];
  projects: number;
  skills: Record<string, number>; // F-AI 매칭: 역량 6축 (skills 라벨과 동일) → 레이더 오버레이
}
...
interface InfraItem {
  ...
  type: "장비" | "공간";
  location: string;
  image: string; // 임시 이미지 URL (로드 실패 시 플레이스홀더로 대체)
}

interface InfraReservation {
  id: string;
  infraId: string;
  infraName: string;
  type: "장비" | "공간";
  date: string; // YYYY-MM-DD
  start: string; // 시작 시각 (예: "10:00")
  end: string; // 종료 시각, 배타 (예: "13:00" → 12시대까지 사용)
  purpose: string;
  headcount: number;
  requester: string;
...
interface DashChart {
  edu: BarDatum[]; // 교육성과 (막대)
  demand: DonutDatum[]; // 산업수요 (도넛)
}
...
interface AppData {
  ...
  skills: Skill[];
  badges: Badge[];
  permRequests: PermRequest[];
  projects: Project[];
  ip: IpItem[];
  talents: Talent[];
  infra: InfraItem[];
  infraSlots: string[];
  infraReservations: InfraReservation[];
  teams: Team[];
  members: Member[];
  adminLogs: AdminLog[];
  ideEnvs: IdeEnv[];
  ...
  learnRecs: LearnRec[];
  notifs: Notif[];
  dashboard: Record<Period, DashChart>;
  companySignups: CompanySignup[];
}
