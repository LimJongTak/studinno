...
/* ============ 인프라 예약 ============ */
type InfraCat = "all" | "장비" | "공간";
let infraCat: InfraCat = "all";
...
function placeholderImg(f: InfraItem): string {
  const [c1, c2] =
    f.type === "장비" ? ["#3b82f6", "#6366f1"] : ["#16bace", "#22d3ee"];
  const svg =
...
function infraImgError(img: HTMLImageElement, id: string): void {
  img.onerror = null;
  const f = DATA.infra.find((x) => x.id === id);
  if (f) img.src = placeholderImg(f);
}

function infraCards(): string {
  const items = DATA.infra.filter(
    (f) => infraCat === "all" || f.type === infraCat
  );
  if (!items.length)
  ...
  return items
    ...
    <div class="card infra-card">
      <img class="infra-thumb" src="${f.image || placeholderImg(f)}" alt="${
        f.name
      }" loading="lazy" onerror="infraImgError(this,'${f.id}')"/>
...
/* ----- 예약 신청 모달 (1.날짜 → 2.시간·목적) ----- */
interface ResvState {
  infraId: string;
  year: number;
  month: number; // 0-11
  date: string | null; // YYYY-MM-DD
  start: string | null; // 시작 시각
  end: string | null; // 종료 시각(배타)
}
let resvState: ResvState | null = null;
let resvPurpose = "";
let resvHeadcount = "1";
let resvSeq = 100;
...
function slotStatus(
  ...
  date: string,
  slot: string
): "free" | "approved" | "pending" {
  const r = DATA.infraReservations.find(
    (x) =>
      x.infraId === infraId &&
      x.date === date &&
      x.status !== "rejected" &&
...
function pickSlot(slot: string): void {
  const st = resvState!;
  ...
  if (!rangeFree(st.infraId, st.date as string, st.start, proposedEnd)) {
...
function openInfraReservation(infraId: string): void {
  const now = new Date();
  resvState = {
    infraId,
...
function buildCalendar(): string {
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    if (st.date === ds) cls.push("selected");
    cells += `<button class="${cls.join(" ")}" data-date="${ds}" ${
      past ? "disabled" : ""
    }>${d}</button>`;
  }
  return `
    <div class="cal">
      ...
      <div class="cal-grid cal-dow">${WEEKDAYS.map(
        (w) => `<div class="cal-w">${w}</div>`
      ).join("")}</div>
      <div class="cal-grid cal-days">${cells}</div>
    </div>`;
}

function buildResvForm(f: InfraItem): string {
  const st = resvState!;
  const slots = DATA.infraSlots
    .map((s) => {
      const status = slotStatus(st.infraId, st.date as string, s);
      const selected =
        st.start !== null && st.end !== null && s >= st.start && s < st.end;
      const cls = ["slot"];
      if (status === "approved") cls.push("booked");
      if (status === "pending") cls.push("pending");
      if (selected) cls.push("selected");
      return `<button class="${cls.join(" ")}" data-time="${s}" ${
        status !== "free" ? "disabled" : ""
      }>${s}</button>`;
    })
    .join("");
  const rangeLabel =
    st.start && st.end ? `${st.start} ~ ${st.end}` : "시작 시간을 선택하세요";
  return `
    <div class="resv-form">
...
function renderResvModal(): void {
  ...
  openModal(`
    ...
    <img class="resv-thumb" src="${f.image || placeholderImg(f)}" alt="${
    f.name
  }" onerror="infraImgError(this,'${f.id}')"/>
    <p class="modal-desc"><span class="muted sm">📍 ${f.location} · ${
    f.type
  }</span><br/>1단계 · 날짜를 선택한 뒤 시간과 사용 목적을 입력하세요.</p>
    ${buildCalendar()}
    ${
      resvState.date
        ? buildResvForm(f)
...
function cacheResvForm(): void {
  const p = document.getElementById("resvPurpose") as HTMLTextAreaElement | null;
  if (p) resvPurpose = p.value;
  const h = document.getElementById("resvHeadcount") as HTMLInputElement | null;
  if (h) resvHeadcount = h.value;
}

function bindResvModalEvents(): void {
  // 달 이동
  document.querySelectorAll<HTMLElement>(".cal-nav").forEach((b) => {
  ...
  // 날짜 선택
  document
    .querySelectorAll<HTMLButtonElement>(".cal-day[data-date]")
    .forEach((b) => {
      b.addEventListener("click", () => {
        if (b.disabled) return;
        cacheResvForm();
        resvState!.date = b.dataset.date as string;
  ...
  // 시간 선택 (연속 범위)
  document
    .querySelectorAll<HTMLButtonElement>(".resv-form .slot[data-time]")
    .forEach((b) => {
      b.addEventListener("click", () => {
        if (b.disabled) return;
        cacheResvForm();
        pickSlot(b.dataset.time as string);
        renderResvModal();
      });
    });
  // 목적/인원 즉시 캐시
  const p = document.getElementById("resvPurpose") as HTMLTextAreaElement | null;
  if (p) p.addEventListener("input", () => (resvPurpose = p.value));
  const h = document.getElementById("resvHeadcount") as HTMLInputElement | null;
  if (h) h.addEventListener("input", () => (resvHeadcount = h.value));
  // 신청
  const sub = document.getElementById("resvSubmit");
  if (sub) sub.addEventListener("click", submitReservation);
}
...
function submitReservation(): void {
  ...
  toast(
    `📅 <b>${f.name}</b> · ${st.date} ${st.start}~${st.end} 예약을 신청했습니다. 관리자 승인 대기 중입니다.`,
    "info"
  );
}

/* ----- 내 예약 조회 ----- */
function resvStatusMeta(s: RequestStatus): { cls: string; label: string } {
  if (s === "approved") return { cls: "approved", label: "✅ 승인됨" };
  if (s === "rejected") return { cls: "rejected", label: "🚫 반려됨" };
  return { cls: "pending", label: "⏳ 승인대기" };
}

function openMyReservationsModal(): void {
  const mine = DATA.infraReservations.filter(
    (r) => r.requester === DATA.user.name
  );
  openModal(`
    <div class="modal-head">
      <h3>📋 내 인프라 예약</h3>
    ...
    <div class="req-list">
      ${
        ...
        <div class="req-item">
          ...
          <div class="req-who">📅 ${r.date} · ⏰ ${r.start}~${r.end} · 👥 ${
                  r.headcount
                }명</div>
...
/* ----- F-MA: AI 추천 과제 찜 토글 ----- */
function bindFavButtons(): void {
  document.querySelectorAll<HTMLElement>(".fav-btn").forEach((b) => {
    b.addEventListener("click", (e) => {
      e.stopPropagation();
      const id = b.dataset.pjt as string;
...
/* ----- F-AD: 기업 회원 가입 승인 ----- */
function adminCompanyCard(): string {
  const pending = DATA.companySignups.filter((c) => c.status === "pending");
  return `
    <div class="card">
      <h3 class="card-title">🏢 기업 회원 가입 승인 <span class="req-count">${
        pending.length
      }</span></h3>
      ${
        pending.length
          ...
          ${pending
            ...
            <tr id="cs-${c.id}">
              ...
              <td class="resv-actions">
                <button class="btn outline sm" onclick="handleCompanyApproval('${
                  c.id
                }','reject')">반려</button>
                <button class="btn primary sm" onclick="handleCompanyApproval('${
                  c.id
                }','approve')">승인</button>
              </td>
            </tr>`
...
function handleCompanyApproval(id: string, action: string): void {
  ...
  if (m && action === "approve") m.status = "활성";
  ...
  toast(
    action === "approve"
      ...
      : `🚫 <b>${c.company}</b> 기업 가입을 반려했습니다.`,
    action === "approve" ? "success" : "warn"
  );
}

/* ----- 관리자: 예약 승인/반려 ----- */
function adminInfraResvCard(): string {
  const pending = DATA.infraReservations.filter((r) => r.status === "pending");
  return `
    <div class="card">
      <h3 class="card-title">🏗️ 인프라 예약 승인 <span class="req-count">${
        pending.length
      }</span></h3>
      ${
        pending.length
        ...
        <tbody>
          ${pending
            ...
            <tr id="resv-${r.id}">
              ...
              <td class="resv-actions">
                <button class="btn outline sm" onclick="handleInfraResv('${
                  r.id
                }','reject')">반려</button>
                <button class="btn primary sm" onclick="handleInfraResv('${
                  r.id
                }','approve')">승인</button>
              </td>
            </tr>`
...
function handleInfraResv(id: string, action: string): void {
  ...
  DATA.adminLogs.unshift({
    time: nowHM(),
    type: "예약",
    msg: `${r.infraName} ${r.date} ${r.start}~${r.end} 예약 ${
