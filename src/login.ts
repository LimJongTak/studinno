/* ============ 로그인 로직 ============ */
const selected = new Set<string>(); // 선택된 서버
const ssoLinked: { gitlab: boolean; kaggle: boolean } = {
  gitlab: false,
  kaggle: false,
};

const hint = document.getElementById("serverHint") as HTMLElement;
const note = document.getElementById("ssoNote") as HTMLElement;
const loginGo = document.getElementById("loginGo") as HTMLButtonElement;

/* --- SSO 버튼 (보완5: 자동 가입·연동) --- */
document.querySelectorAll<HTMLButtonElement>(".sso-btn").forEach((btn) => {
  btn.addEventListener("click", () => {
    const sso = btn.dataset.sso as "gitlab" | "kaggle";
    ssoLinked[sso] = true;
    btn.innerHTML = `<span class="sso-ic">✅</span> ${
      sso === "gitlab" ? "GitLab" : "Kaggle"
    } 연동 완료`;
...
/* --- 서버 선택 (보완7) --- */
document
  .querySelectorAll<HTMLInputElement>(".server-opt input")
  .forEach((cb) => {
