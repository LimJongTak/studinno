...
function drawMatchRadar(canvasId, req, skills) {
    ...
    for (let lvl = 1; lvl <= 4; lvl++) {
        ctx.beginPath();
        for (let i = 0; i <= n; i++) {
            const a = ((Math.PI * 2) / n) * i - Math.PI / 2, r = (R / 4) * lvl;
    ...
    const drawSeries = (values, stroke, fill) => {
        ctx.beginPath();
        values.forEach((v, i) => {
            const a = ((Math.PI * 2) / n) * i - Math.PI / 2, r = R * (v / 100);
            const x = cx + r * Math.cos(a), y = cy + r * Math.sin(a);
            i === 0 ? ctx.moveTo(x, y) : ctx.lineTo(x, y);
        });
        ctx.closePath();
    ...
    labels.forEach((l, i) => {
        const a = ((Math.PI * 2) / n) * i - Math.PI / 2;
        const x = cx + (R + 16) * Math.cos(a), y = cy + (R + 12) * Math.sin(a);
        ctx.fillText(l, x, y);
    });
}
function eduBars(period) {
    ...
    <div class="bar-row" title="${d.label}: ${d.value}">
      ...
      <div class="bar-track"><div class="bar-val" style="width:${Math.round((d.value / max) * 100)}%"></div></div>
      <span class="bar-num">${d.value}</span>
    </div>`)
        .join("");
}
function donutLegend(period) {
...
function drawDemandDonut(period) {
    const cv = document.getElementById("demandDonut");
    if (!cv)
        return;
    const ctx = cv.getContext("2d");
    if (!ctx)
    ...
    const data = DATA.dashboard[period].demand;
    const total = data.reduce((s, d) => s + d.value, 0);
    const cx = 85, cy = 85, rOuter = 72, rInner = 44;
...
function bindDonutTooltip() {
    ...
    cv.onmouseleave = () => tipEl.classList.remove("show");
}
/* ============ 역량 레이더 차트 ============ */
function drawRadar() {
    ...
    const data = DATA.skills, n = data.length;
    ...
    ctx.beginPath();
    data.forEach((d, i) => {
        const a = ((Math.PI * 2) / n) * i - Math.PI / 2, r = R * (d.value / 100);
        const x = cx + r * Math.cos(a), y = cy + r * Math.sin(a);
        i === 0 ? ctx.moveTo(x, y) : ctx.lineTo(x, y);
    });
    ctx.closePath();
...
/* ============ 진로 로드맵 레이더 (현재 vs 목표) ============ */
function drawRoadmapRadar() {
    const cv = document.getElementById("roadmapRadar");
    if (!cv)
        return;
    const ctx = cv.getContext("2d");
    if (!ctx)
    ...
    const drawSeries = (values, stroke, fill, width) => {
        ctx.beginPath();
        values.forEach((v, i) => {
            const a = ((Math.PI * 2) / n) * i - Math.PI / 2, r = R * (v / 100);
            const x = cx + r * Math.cos(a), y = cy + r * Math.sin(a);
...
function drawAdminMonitor() {
    const cv = document.getElementById("adminMonitor");
    if (!cv)
        return;
    const ctx = cv.getContext("2d");
    if (!ctx)
    ...
    const padL = 32, padR = 14, padT = 14, padB = 20;
    const plotW = W - padL - padR, plotH = H - padT - padB;
    ctx.clearRect(0, 0, W, H);
    const css = (v, d) => getComputedStyle(document.body).getPropertyValue(v).trim() || d;
