...
function branchOptions(pjt) {
    const list = DATA.branches.filter((b) => b.project === pjt);
    return list
        .map((b) => `<option value="${b.name}">${b.name}${b.isDefault ? " (default)" : ""}</option>`)
        .join("");
}
function repoTabs() {
    const openMr = DATA.mergeRequests.filter((m) => m.project === currentWorkspace && (m.status === "open" || m.status === "draft")).length;
    const openIssues = DATA.issues.filter((i) => i.project === currentWorkspace && i.status === "open").length;
    const tabs = [
        ["commits", "💾 커밋"],
        ["branches", "🌿 브랜치"],
        ["mr", `🔀 MR${openMr ? " " + openMr : ""}`],
    ...
    return tabs
        .map(([k, label]) => `<button class="cat-tab ${ideTab === k ? "active" : ""}" data-repotab="${k}">${label}</button>`)
        .join("");
}
function repoPanel() {
    const pjt = currentWorkspace;
    if (ideTab === "branches")
        return branchRows(pjt);
    if (ideTab === "mr")
        return mrRows(pjt);
    if (ideTab === "issues")
        return issueRows(pjt);
    if (ideTab === "pipelines")
        return pipelineRows(pjt);
    return `<div class="commit-list">${commitRows(pjt)}</div>`;
}
/* ----- 브랜치 ----- */
function branchRows(pjt) {
    const list = DATA.branches.filter((b) => b.project === pjt);
    if (!list.length)
        return emptyState("브랜치가 없습니다.", "🌿");
...
/* ----- 머지 리퀘스트 ----- */
function mrStatusMeta(s) {
    if (s === "merged")
        return { cls: "done", label: "✔ Merged" };
    if (s === "draft")
        return { cls: "info", label: "✎ Draft" };
    if (s === "closed")
        return { cls: "open", label: "✕ Closed" };
    return { cls: "open", label: "● Open" };
}
function pipeBadge(p) {
    if (p === "passed")
        return `<span class="pipe-badge passed">● 통과</span>`;
    if (p === "failed")
        return `<span class="pipe-badge failed">● 실패</span>`;
    if (p === "running")
        return `<span class="pipe-badge running">● 실행중</span>`;
    return "";
}
function linkedTaskTag(taskId) {
...
function mrRows(pjt) {
    ...
    if (!list.length)
        return emptyState("머지 리퀘스트가 없습니다.", "🔀");
    ...
    <div class="mr-row" data-mr="${m.id}">
      <div class="mr-main">
        ...
        <div class="mr-meta muted sm">
          ...
          · <b class="add">+${m.added}</b> <b class="del">−${m.removed}</b> ${pipeBadge(m.pipeline)}${m.task ? linkedTaskTag(m.task) : ""}
        </div>
      </div>
...
function openMrDetail(id) {
    ...
    if (!m)
        return;
    const sm = mrStatusMeta(m.status);
    const canMerge = m.status === "open" && m.approvals >= m.approvalsNeeded && m.pipeline === "passed";
    openModal(`
...
<span class="dl add">+     return self.base(x)</span></pre>
    ...
    <p class="muted sm">${m.status === "merged"
        ? "이미 병합된 MR입니다."
        : canMerge
            ? "✅ 승인 조건과 파이프라인을 충족해 병합할 수 있습니다."
            : "⚠️ 승인 수 또는 파이프라인 통과가 필요합니다."}</p>
...
function approveMr(id) {
    ...
    if (!m || m.status !== "open")
        return;
    if (m.approvals < m.approvalsNeeded)
        m.approvals++;
...
/* ----- 이슈 ----- */
function issueLabelMeta(label) {
    if (label === "bug")
        return { cls: "lb-bug", txt: "🐞 bug" };
    if (label === "feature")
        return { cls: "lb-feat", txt: "✨ feature" };
    return { cls: "lb-docs", txt: "📄 docs" };
}
function issueRows(pjt) {
    const list = DATA.issues.filter((i) => i.project === pjt);
    if (!list.length)
        return emptyState("이슈가 없습니다.", "🐞");
...
function toggleIssue(id) {
    ...
    toast(i.status === "closed" ? `✓ 이슈 <b>${i.id}</b> 닫음` : `○ 이슈 <b>${i.id}</b> 다시 열기`, "info");
    if (currentPage === "ide")
        refreshRepoPanel();
}
/* ----- CI/CD 파이프라인 ----- */
function stageIcon(s) {
    if (s === "passed")
        return `<span class="stg passed" title="통과">✔</span>`;
    if (s === "failed")
        return `<span class="stg failed" title="실패">✕</span>`;
    if (s === "running")
        return `<span class="stg running" title="실행중">◐</span>`;
    return `<span class="stg skipped" title="건너뜀">○</span>`;
}
function pipelineRows(pjt) {
    const list = DATA.pipelines.filter((p) => p.project === pjt);
    if (!list.length)
        return emptyState("파이프라인 실행 기록이 없습니다.", "🚦");
    return `<div class="pipe-list">${list
        .map((p) => `
    <div class="pipe-row">
      <div class="pipe-id">
        <span class="pipe-badge ${p.status}">● ${p.status === "passed" ? "통과" : p.status === "failed" ? "실패" : "실행중"}</span>
        <b>${p.id}</b>
      </div>
      <div class="pipe-main">
        <div class="pipe-stages">${p.stages
        .map((s) => `${stageIcon(s.status)}<span class="stg-name">${s.name}</span>`)
...
/* ----- 패널 갱신 + 바인딩 ----- */
function refreshRepoPanel() {
    const panel = document.getElementById("repoPanel");
    if (panel)
        panel.innerHTML = repoPanel();
    const tabs = document.getElementById("repoTabs");
    if (tabs)
        tabs.innerHTML = repoTabs();
    bindRepoTabs();
    bindRepoPanel();
}
function bindRepoTabs() {
    document.querySelectorAll("#repoTabs .cat-tab").forEach((tab) => {
        tab.addEventListener("click", () => {
            ideTab = tab.dataset.repotab;
            refreshRepoPanel();
        });
    });
}
function bindRepoPanel() {
    // 커밋 Diff
    document.querySelectorAll("#repoPanel .diff-btn").forEach((b) => {
        b.addEventListener("click", () => openDiffModal(b.dataset.id));
    });
    // MR
    document.querySelectorAll("#repoPanel .mr-detail-btn").forEach((b) => {
        b.addEventListener("click", () => openMrDetail(b.dataset.mr));
    });
    document.querySelectorAll("#repoPanel .new-mr-btn").forEach((b) => {
        b.addEventListener("click", () => toast(`🔀 <b>${b.dataset.src}</b> → main MR 생성 화면으로 이동합니다. (데모)`, "info"));
    });
    // 이슈
    document.querySelectorAll("#repoPanel .issue-toggle-btn").forEach((b) => {
        b.addEventListener("click", () => toggleIssue(b.dataset.issue));
    });
    document.querySelectorAll("#repoPanel .new-issue-btn").forEach((b) => {
        b.addEventListener("click", () => toast("🐞 새 이슈 작성 화면으로 이동합니다. (데모)", "info"));
    });
    // 파이프라인
    document.querySelectorAll("#repoPanel .pipe-rerun-btn").forEach((b) => {
        b.addEventListener("click", () => rerunPipeline(b.dataset.pipe));
    });
    // 브랜치
    document.querySelectorAll("#repoPanel .new-branch-btn").forEach((b) => {
