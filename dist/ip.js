...
function bindIpCards() {
    document.querySelectorAll("#ipGrid .ip-detail-btn").forEach((btn) => {
        btn.addEventListener("click", (e) => {
            e.stopPropagation();
            openIpDetail(btn.dataset.ip);
        });
    });
    document.querySelectorAll("#ipGrid .ribbon-btn").forEach((btn) => {
        btn.addEventListener("click", (e) => {
            e.stopPropagation();
            toggleIpBookmark(btn.dataset.ip);
        });
    });
    // del-btn은 초기엔 전역 바인딩이 처리 → 리렌더 시 refreshIpGrid에서 재바인딩
}
function refreshIpGrid() {
    ...
    if (cnt)
        cnt.textContent = String(ipBookmarks.size);
}
function toggleIpBookmark(id) {
    if (ipBookmarks.has(id))
        ipBookmarks.delete(id);
    else {
        ipBookmarks.add(id);
        const x = DATA.ip.find((i) => i.id === id);
        addNotif("🔖", `성과물 <b>${(x === null || x === void 0 ? void 0 : x.title) || ""}</b> 북마크`);
    }
    refreshIpGrid();
}
function similarIp(x) {
    return DATA.ip
...
function openIpDetail(id) {
    ...
    <div class="owners">
      ${x.owners
        .map((o) => `<div class="owner-row"><span>${o.type === "기업" ? "🏢" : "🎓"} ${o.name} <span class="muted sm">(${o.type})</span></span><span class="share">${o.share}%</span></div>`)
        .join("")}
    </div>
    ${sims.length
        ? `<h4 class="mini-title">유사 성과물</h4>
    <div class="sim-track">${sims
            .map((s) => `<div class="sim-card" data-ip="${s.id}"><div class="sim-type">${s.type}</div><div class="sim-title">${s.title}</div><div class="tags">${s.tags
            .slice(0, 3)
            .map((t) => `<span class="tag">#${t}</span>`)
            .join("")}</div></div>`)
            .join("")}</div>`
    ...
    // 유사 성과물 클릭 → 해당 상세로 이동
    modalBox.querySelectorAll(".sim-card").forEach((c) => {
        c.addEventListener("click", () => openIpDetail(c.dataset.ip));
    });
}
function downloadIp(id) {
    const x = DATA.ip.find((i) => i.id === id);
    toast(`⬇ <b>${(x === null || x === void 0 ? void 0 : x.title) || "성과물"}</b> 원문(PDF)을 다운로드합니다. (데모)`, "success");
}
function openBookmarksModal() {
    ...
    <div class="bm-list">
      ${items
        .map((x) => `<div class="bm-row" data-ip="${x.id}"><span class="badge ${x.type === "특허" ? "open" : "info"}">${x.type}</span><b>${x.title}</b><span class="bm-go">상세 ›</span></div>`)
        .join("")}
    </div>
    <div class="modal-actions"><button class="btn primary" onclick="closeModal()">확인</button></div>`);
    modalBox.querySelectorAll(".bm-row").forEach((r) => {
        r.addEventListener("click", () => openIpDetail(r.dataset.ip));
    });
}
