...
(() => {
type SignupType = "student" | "prof" | "company";
...
let curStep = 1;
let signupType: SignupType | null = null;
...
function showStep(n: number): void {
  ...
  document.querySelectorAll<HTMLElement>(".step-bar .step").forEach((s) => {
...
/* ---------- STEP 1: 약관 동의 ---------- */
const agreeAll = $<HTMLInputElement>("agreeAll");
const agrees = Array.from(document.querySelectorAll<HTMLInputElement>(".agree"));
const toStep2 = $<HTMLButtonElement>("toStep2");

function refreshAgree(): void {
...
const docBox = $("docBox");
document.querySelectorAll<HTMLElement>(".agree-view").forEach((b) => {
...
docOverlay.addEventListener("click", (e) => {
  if (e.target === docOverlay) docOverlay.classList.remove("show");
});

/* ---------- STEP 2: 유형 선택 ---------- */
const toStep3 = $<HTMLButtonElement>("toStep3");
document.querySelectorAll<HTMLElement>(".type-card").forEach((card) => {
  card.addEventListener("click", () => {
    document.querySelectorAll(".type-card").forEach((c) => c.classList.remove("active"));
    card.classList.add("active");
    signupType = card.dataset.type as SignupType;
...
/* ---------- STEP 3: 동적 폼 + 검증 ---------- */
function buildDynamicFields(): void {
  ...
  host.innerHTML = html;
  host
    .querySelectorAll<HTMLInputElement>(".dyn-req")
    .forEach((i) => i.addEventListener("input", validateStep3));
}

/* 이메일 인증 타이머 3:00 */
const sendCode = $<HTMLButtonElement>("sendCode");
const checkCode = $<HTMLButtonElement>("checkCode");
...
sendCode.addEventListener("click", () => {
  const email = $<HTMLInputElement>("suEmail").value.trim();
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
...
checkCode.addEventListener("click", () => {
  const code = $<HTMLInputElement>("suCode").value.trim();
  if (!/^\d{6}$/.test(code)) {
...
/* 비밀번호 실시간 유효성 */
const pw = $<HTMLInputElement>("suPw");
const pw2 = $<HTMLInputElement>("suPw2");

function pwRules(v: string): Record<string, boolean> {
  return {
    len: v.length >= 8,
...
function pwValid(v: string): boolean {
  return Object.values(pwRules(v)).every(Boolean);
}
function refreshPw(): void {
  const rules = pwRules(pw.value);
  document.querySelectorAll<HTMLElement>("#pwCheck li").forEach((li) => {
    li.classList.toggle("ok", rules[li.dataset.rule as string]);
  });
  const m = $("pwMatch");
  if (!pw2.value) {
    m.textContent = "";
...
function validateStep3(): void {
  const nameOk = $<HTMLInputElement>("suName").value.trim() !== "";
  const dynOk = Array.from(
    document.querySelectorAll<HTMLInputElement>("#dynamicFields .dyn-req")
  ).every((i) => i.value.trim() !== "");
  const pwOk = pwValid(pw.value) && pw.value === pw2.value && pw2.value !== "";
  $<HTMLButtonElement>("signupGo").disabled = !(nameOk && dynOk && emailVerified && pwOk);
}
...
$("signupGo").addEventListener("click", () => {
  const name = $<HTMLInputElement>("suName").value.trim();
  const record = {
    name,
    type: signupType,
    email: $<HTMLInputElement>("suEmail").value.trim(),
    dept: ($<HTMLInputElement>("suDept") || ({} as HTMLInputElement)).value || "",
  };
  ...
  if (isCompany) {
    ...
    const entry = {
      id: "CS-" + Date.now(),
      company: ($<HTMLInputElement>("suCompany") || ({} as HTMLInputElement)).value.trim() || name,
      biz: ($<HTMLInputElement>("suBiz") || ({} as HTMLInputElement)).value.trim() || "-",
      manager: name,
      dept: ($<HTMLInputElement>("suDept") || ({} as HTMLInputElement)).value.trim() || "-",
      date: today,
      status: "pending",
    };
    try {
