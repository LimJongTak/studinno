...
function compStatusMeta(s: CompStatus): { cls: string; label: string } {
  if (s === "진행중") return { cls: "open", label: "🟢 진행중" };
  if (s === "예정") return { cls: "info", label: "🔜 예정" };
  return { cls: "done", label: "🏁 종료" };
}
function compFilterTabs(): string {
  ...
  return tabs
    .map(
      ([k, label]) =>
        `<button class="cat-tab ${compFilter === k ? "active" : ""}" data-cf="${k}">${label}</button>`
    )
    .join("");
}
function filteredComps(): Competition[] {
  return DATA.competitions.filter((c) => {
    if (compFilter === "mine") return c.joined;
    if (compFilter === "경진대회" || compFilter === "해커톤") return c.type === compFilter;
    return true;
  });
}
function myRank(c: Competition): LeaderRow | undefined {
  return c.leaderboard.find((r) => r.isMe);
}
function teamLabel(c: Competition): string {
  return c.teamMax > 1 ? `👥 ${c.teamMin}~${c.teamMax}인 팀` : "👤 개인전";
}
function compCard(c: Competition): string {
  const sm = compStatusMeta(c.status);
  const mine = myRank(c);
  return `
  <div class="card comp-card" data-comp="${c.id}">
    <div class="comp-top">
      <span class="comp-type ${c.type === "해커톤" ? "hack" : "race"}">${
    c.type === "해커톤" ? "⚡ 해커톤" : "🏆 경진대회"
  }</span>
    ...
    ${
      c.joined && c.myTeam
        ? `<div class="comp-myrank">🧑‍🤝‍🧑 <b>${c.myTeam.name}</b> (${c.myTeam.members.length}명)${
            mine ? ` · 현재 <b>${mine.rank}위</b> ${rankChange(mine.change)}` : ""
          }</div>`
        : mine
...
function rankChange(ch: number): string {
  if (ch > 0) return `<span class="rc up">▲${ch}</span>`;
  if (ch < 0) return `<span class="rc down">▼${-ch}</span>`;
  return `<span class="rc same">–</span>`;
}
function compGrid(): string {
  const list = filteredComps();
  if (!list.length) return emptyState("해당 조건의 대회가 없습니다.", "🏆");
  return list.map(compCard).join("");
}
function leaderboardTable(c: Competition): string {
  ...
  return `<table class="tbl lb-tbl">
    ...
    <tbody>
      ${c.leaderboard
        .map(
          (r) =>
            `<tr class="${r.isMe ? "lb-me" : ""}"><td class="lb-rank ${
              r.rank <= 3 ? "top" : ""
            }">${r.rank <= 3 ? ["🥇", "🥈", "🥉"][r.rank - 1] : r.rank}</td><td>${
              r.team
            }${r.isMe ? ' <span class="lb-metag">나</span>' : ""}</td><td class="muted sm">${
              r.members
            }명</td><td><b>${r.score}</b></td><td class="muted sm">${
              r.entries
            }</td><td>${rankChange(r.change)}</td></tr>`
        )
        .join("")}
    </tbody>
  </table>`;
}
function submissionRows(): string {
...
function phaseBar(c: Competition): string {
  const phases: CompPhase[] = ["모집", "진행", "채점", "발표"];
  const cur = phases.indexOf(c.phase);
  return `<div class="phase-bar">${phases
    .map(
      (p, i) =>
        `<div class="phase-step ${i < cur ? "done" : i === cur ? "active" : ""}"><span class="ps-dot">${
          i < cur ? "✓" : i + 1
        }</span><span class="ps-label">${p}</span></div>`
    )
    .join('<div class="phase-line"></div>')}</div>`;
}
function teamRoster(c: Competition): string {
  if (!c.myTeam) return "";
  return `<h4 class="mini-title">🧑‍🤝‍🧑 내 팀 — ${c.myTeam.name} <span class="muted sm">(${c.myTeam.members.length}/${c.teamMax}명)</span></h4>
    <div class="roster">${c.myTeam.members
      .map(
        (m, i) =>
          `<span class="roster-chip ${i === 0 ? "leader" : ""}">${i === 0 ? "👑 " : ""}${m}</span>`
      )
      .join("")}</div>`;
}
function openCompDetail(id: string): void {
  ...
  openModal(`
    ...
    <div class="prize-list">${c.prizes
      .map(
        (p) => `<div class="prize-row"><span class="pz-place">${p.place}</span><span>${p.reward}</span></div>`
      )
      .join("")}</div>
...
// ----- 팀 단위 참가 신청 모달 -----
function openJoinModal(id: string): void {
  const c = DATA.competitions.find((x) => x.id === id);
  ...
  openModal(`
    ...
    <p class="modal-desc">${
      c.type === "해커톤" ? "⚡ 해커톤은 팀 단위 참가입니다. " : ""
    }팀 구성 규정: <b>${c.teamMin}~${c.teamMax}명</b></p>
    ...
    <div id="teamFields">
      ...
      <div class="member-pick">
        ${candidates
          .map(
            (n) =>
              `<label class="mp-item"><input type="checkbox" class="mp-chk" value="${n}"/> ${n}</label>`
          )
          .join("")}
      </div>
  ...
  const teamFields = modalBox.querySelector("#teamFields") as HTMLElement | null;
  const countEl = modalBox.querySelector("#teamCount b") as HTMLElement | null;
  const refreshCount = (): void => {
    const n = 1 + modalBox.querySelectorAll<HTMLInputElement>(".mp-chk:checked").length;
    if (countEl) countEl.textContent = String(n);
  };
  modalBox.querySelectorAll<HTMLInputElement>('input[name="jmode"]').forEach((r) => {
    r.addEventListener("change", () => {
      if (teamFields) teamFields.style.display = r.value === "solo" ? "none" : "";
    });
  });
  modalBox.querySelectorAll<HTMLInputElement>(".mp-chk").forEach((cb) => {
    cb.addEventListener("change", refreshCount);
  });
}
function confirmJoin(id: string, btn?: HTMLButtonElement): void {
  const c = DATA.competitions.find((x) => x.id === id);
  if (!c || c.joined) return;
  const modeEl = modalBox.querySelector(
    'input[name="jmode"]:checked'
  ...
  if (mode === "team") {
    ...
    const picked = Array.from(
      modalBox.querySelectorAll<HTMLInputElement>(".mp-chk:checked")
    ).map((el) => el.value);
  ...
  busy(btn, () => {
    ...
    addNotif(
      "🏆",
      `대회 참가 신청: <b>${c.title}</b> — ${
        mode === "team" ? `${teamNm} (${members.length}명)` : "개인"
      }`
    );
    toast(
      `✅ <b>${c.title}</b> 에 ${
        mode === "team" ? `팀 <b>${teamNm}</b>(${members.length}명)으로` : "개인으로"
      } 참가 신청했습니다.`,
      "success"
    );
    closeModal();
    if (currentPage === "compete") render("compete");
  });
}
function openSubmitModal(id: string): void {
  const c = DATA.competitions.find((x) => x.id === id);
  if (!c) return;
  openModal(`
...
function submitEntry(id: string, btn?: HTMLButtonElement): void {
  const c = DATA.competitions.find((x) => x.id === id);
  if (!c) return;
  ...
  busy(
    ...
    () => {
      ...
      c.leaderboard.forEach((r, i) => {
        const newRank = i + 1;
        if (r.isMe) r.change = r.rank - newRank; // 내 순위 변동만 갱신(타 팀 기존 변동 보존)
        r.rank = newRank;
      });
...
function talentCompAwards(name: string): { title: string; rank: number }[] {
  const out: { title: string; rank: number }[] = [];
  DATA.competitions.forEach((c) => {
    if (c.myTeam && c.myTeam.members.includes(name)) {
      const me = c.leaderboard.find((r) => r.isMe);
      if (me && me.rank <= 3) out.push({ title: c.title, rank: me.rank });
    }
  });
  return out;
}
function talentCompBonus(name: string): number {
  return Math.min(
    12,
    talentCompAwards(name).reduce((s, a) => s + (4 - a.rank) * 2, 0)
  );
}
function compAwardBadge(name: string): string {
...
function myCompStats(): { joined: number; awards: number } {
...
function compAchievements(): string {
  const joined = DATA.competitions.filter((c) => c.joined);
  if (!joined.length)
  ...
  return `<div class="comp-ach-list">${joined
    .map((c) => {
      ...
      return `<div class="comp-ach-row ${award ? "award" : ""}">
        ...
        <div class="ca-rank">${medal} <b>${m ? m.rank + "위" : "-"}</b>${
        m ? ` <span class="muted sm">/ ${c.participants}팀</span>` : ""
      }</div>
...
function bindCompete(): void {
  document.querySelectorAll<HTMLElement>("#compFilterTabs .cat-tab").forEach((tab) => {
    tab.addEventListener("click", () => {
      compFilter = tab.dataset.cf as string;
      document
        .querySelectorAll("#compFilterTabs .cat-tab")
        .forEach((t) => t.classList.remove("active"));
      tab.classList.add("active");
      const grid = document.getElementById("compGrid");
      if (grid) grid.innerHTML = compGrid();
      bindCompCards();
    });
  });
  bindCompCards();
}
function bindCompCards(): void {
  document.querySelectorAll<HTMLElement>("#compGrid .comp-detail-btn").forEach((b) => {
    b.addEventListener("click", () => openCompDetail(b.dataset.comp as string));
  });
  document.querySelectorAll<HTMLElement>("#compGrid .comp-join-btn").forEach((b) => {
    b.addEventListener("click", () => openJoinModal(b.dataset.comp as string));
  });
  document.querySelectorAll<HTMLElement>("#compGrid .comp-submit-btn").forEach((b) => {
    b.addEventListener("click", () => openSubmitModal(b.dataset.comp as string));
  });
}
