...
const PAGES: Record<PageKey, () => string> = {
  ...
  projects: () => `
    <div class="toolbar">
      ...
      <button class="btn outline" id="incomingReqBtn">
        🔔 받은 요청 <span class="req-count">${
          DATA.permRequests.filter((r) => r.status === "pending").length
        }</span>
      </button>
  ...
  workspace: () => {
    const p =
      DATA.projects.find((x) => x.id === currentWorkspace) || DATA.projects[0];
    ...
    <div class="card ws-head">
      <div>
        ...
        <p class="muted sm">${p.company} · 👥 ${p.members.join(
      ", "
    )} · 진행률 ${p.progress}%</p>
      </div>
      <button class="btn ${
        p.status === "완료" ? "outline" : "primary"
      } ws-complete-btn" data-pjt="${p.id}" ${
      p.status === "완료" ? "disabled" : ""
    }>${p.status === "완료" ? "✅ 완료됨" : "🏁 프로젝트 완료"}</button>
    </div>
  ...
  ide: () => `
    ...
    <div class="card">
      <div class="repo-head">
        <h3 class="card-title">🦊 GitLab 저장소 <span class="muted sm">(${pjtTitle(
          currentWorkspace
        )})</span></h3>
        <div class="repo-branch">🌿 브랜치
          <select class="input" id="repoBranchSel">${branchOptions(
            currentWorkspace
          )}</select>
        </div>
      </div>
    ...
    <div class="card portfolio-card">
      ...
      <div class="pf-body">
        <div class="pf-summary">
          ...
          <div class="pf-stat"><b>${
            DATA.projects.filter((p) => p.members.includes(DATA.user.name))
              .length
          }</b><span>참여 프로젝트</span></div>
          <div class="pf-stat"><b>${
            DATA.ip.filter((x) =>
              x.owners.some((o) => o.name === DATA.user.name)
            ).length
          }</b><span>등록 IP</span></div>
  ...
  roadmap: () => {
    ...
    <div class="grid-2">
      <div class="card">
        <h3 class="card-title">📊 역량 진단 <span class="muted sm">(현재 vs ${
          job.name
        } 목표)</span></h3>
      ...
      ${
        ...
        <div class="card">
          <div class="pjt-head">
            ...
            <span class="badge ${t.recruiting ? "open" : "done"}">${
              t.recruiting ? "모집중" : "구성완료"
            }</span>
            ${delBtn("team", t.id)}
          </div>
          ...
          ${
            t.recruiting
              ? `<div class="need">구인: ${t.need
                  ...
                  .join("")}</div>
            <button class="btn outline full apply-team" data-team="${t.id}" ${
                  appliedTeams.has(t.id) ? "disabled" : ""
                }>${appliedTeams.has(t.id) ? "✓ 지원 완료" : "지원하기"}</button>`
              : ""
          }
        </div>`
              )
  ...
  infra: () => `
    <div class="toolbar infra-toolbar">
      <div class="cat-tabs" id="infraCatTabs">
        ...
        <button class="cat-tab ${
          infraCat === "장비" ? "active" : ""
        }" data-cat="장비">🖥️ 장비</button>
        <button class="cat-tab ${
          infraCat === "공간" ? "active" : ""
        }" data-cat="공간">🏛️ 공간</button>
      </div>
      <button class="btn outline" id="myResvBtn">📋 내 예약 <span class="req-count">${
        DATA.infraReservations.filter((r) => r.requester === DATA.user.name)
          .length
      }</span></button>
    </div>
  ...
  admin: () => `
    <div class="grid-3">
      ...
      ${statCard(
        "예약 승인 대기",
        String(
          DATA.infraReservations.filter((r) => r.status === "pending").length
        ),
        "⏳"
      )}
      ${statCard(
        "기업 승인 대기",
        String(DATA.companySignups.filter((c) => c.status === "pending").length),
        "🏢"
      )}
    </div>
    ${adminChartCard()}
