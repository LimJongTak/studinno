/* ============ 로그인 결과 반영 ============ */
(function loadLoginState(): void {
  const servers: string[] = JSON.parse(
    localStorage.getItem("studinno-servers") || '["STUDINNO"]'
  );
  const badge = document.getElementById("serverBadge");
  if (badge) badge.textContent = "🟢 " + servers.join(" + ");

  // SSO 연동 상태 → 사이드바 하단 뱃지
  const sso: { gitlab?: boolean; kaggle?: boolean } = JSON.parse(
    localStorage.getItem("studinno-sso") || "{}"
  );
  document.querySelectorAll<HTMLElement>(".lb").forEach((lb) => {
...
themeToggle.addEventListener("click", () => {
  ...
  if (currentPage === "dashboard") drawRadar();
});
...
let dashPeriod: Period = "3m";
const favProjects = new Set<string>();
...
let ipQuery = "";
let ipFilter = "all";
const ipBookmarks = new Set<string>();
// F-AI 인재검색: 관심인재 찜
const favTalents = new Set<string>();
...
const adminSettings: Record<string, boolean> = {
  tls: true,
  encrypt: true,
  twofa: false,
};
// AI 매칭 엔진: 관점 토글(학생/기업) + 기업 관점에서 선택한 RFP + 이미 지원/제안한 매칭
let matchView: "student" | "company" = "student";
let matchRfp = "PJT-001";
const matchActed = new Set<string>(); // `${pjtId}:${talent}` 키
// 인재검색 블라인드(공정 평가): 이름·아바타 마스킹
let talentBlind = false;
// 등록/제안 CTA 상태 (제안 보낸 인재 / 지원한 팀 / 이번 세션에 직접 등록한 항목)
const proposedTalents = new Set<string>();
const appliedTeams = new Set<string>();
const createdItems = new Set<string>();
const recentSearches: string[] = ["스마트팩토리", "NLP 챗봇", "GPU 예약"];
const app = document.getElementById("app") as HTMLElement;
let currentPage: PageKey = "dashboard";

// Enter/Space 로도 클릭되게 (키보드 접근성)
function keyActivate(el: HTMLElement): void {
  el.tabIndex = 0;
...
document.querySelectorAll<HTMLElement>(".nav-item").forEach((item) => {
  ...
  item.addEventListener("click", () => {
    goPage(item.dataset.page as PageKey); // active 동기화 + 스켈레톤 전환
  });
});
// GNB 호버 드롭다운: 플라이아웃 항목 클릭 → 해당 페이지 (부모 nav-item 이동 방지)
document.querySelectorAll<HTMLElement>(".fly-item").forEach((fi) => {
  keyActivate(fi);
  fi.addEventListener("click", (e) => {
    e.stopPropagation();
    goPage(fi.dataset.page as PageKey);
  });
});
...
document.addEventListener("click", () => sidebarEl?.classList.remove("open"));

function render(page: PageKey): void {
  currentPage = page;
  (document.getElementById("pageTitle") as HTMLElement).textContent =
    TITLES[page];
  ...
  if (page === "roadmap") drawRoadmapRadar();
  if (page === "admin") drawAdminMonitor();
  bindPageEvents(page);
  document.getElementById("sidebar")?.classList.remove("open"); // 모바일: 이동 후 닫기
}
...
let navSeq = 0;
function goPage(page: PageKey): void {
  document.querySelectorAll<HTMLElement>(".nav-item").forEach((n) => {
    ...
    if (owns) n.setAttribute("aria-current", "page");
  ...
  window.setTimeout(() => {
    if (seq === navSeq) render(page);
  }, 420);
}

/* ============ 헬퍼 ============ */
function statCard(label: string, val: string, ic: string): string {
  return `
...
function projectFilterTabs(): string {
  ...
  return tabs
    .map(
      ([k, label]) =>
        `<button class="cat-tab ${projectFilter === k ? "active" : ""}" data-pf="${k}">${label}</button>`
    )
    .join("");
}
function filteredProjects(): Project[] {
  ...
  return DATA.projects.filter((p) => {
    if (hiddenProjects.has(p.id)) return false; // 관리자가 숨김 처리한 프로젝트 비노출
  ...
  return `
        <div class="card pjt">
          <div class="pjt-head">
            ...
            <span class="ide-btn">${
              p.ide === "gitlab" ? "🦊 GitLab IDE" : "📊 Kaggle"
            }</span>
            ${delBtn("project", p.id)}
          </div>
          ...
          <div class="repo-meta">
            ...
            <span class="vis-badge ${p.visibility}">${
...
function bindProjectCards(): void {
  document.querySelectorAll<HTMLElement>("#projectGrid .file-row").forEach((row) => {
    row.addEventListener("click", () => {
      const perm = row.dataset.perm as FilePerm;
      const file = row.dataset.file as string;
      const pjt = row.dataset.pjt as string;
      if (perm === "public") toast(`📄 <b>${file}</b> 열람을 시작합니다.`, "success");
      else openRequestModal(pjt, file, perm);
    });
  });
  document.querySelectorAll<HTMLElement>("#projectGrid .perm-edit-btn").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      e.stopPropagation();
      openPermEditModal(btn.dataset.pjt as string);
    });
  });
  document.querySelectorAll<HTMLElement>("#projectGrid .ws-enter-btn").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      e.stopPropagation();
      currentWorkspace = btn.dataset.pjt as string;
      goPage("workspace");
    });
  });
  // del-btn은 초기엔 bindPageEvents 상단의 전역 바인딩이 처리(중복 방지) → 리렌더 시에만 재바인딩
}
function refreshProjectGrid(): void {
...
function talentCard(t: Talent, idx: number = 0): string {
  ...
  <div class="card talent-card${talentBlind ? " blind" : ""}">
    ...
    <button class="btn outline full propose-btn" data-name="${t.name}" ${
    proposedTalents.has(t.name) ? "disabled" : ""
  }>${proposedTalents.has(t.name) ? "✓ 제안 완료" : "제안하기"}</button>
  </div>`;
}
function toggleFavTalent(name: string): void {
  if (favTalents.has(name)) favTalents.delete(name);
  else {
    favTalents.add(name);
    addNotif("⭐", `관심 인재 등록: <b>${name}</b>`);
  }
  const btn = document.querySelector<HTMLElement>(`.talent-fav[data-fav="${name}"]`);
  if (btn) {
...
const stClass = (s: string): string =>
  s === "진행중" ? "open" : s === "완료" ? "done" : "info";
const permIcon = (p: string): string =>
  p === "public" ? "🌐" : p === "private" ? "🔒" : "📨";
const permLabel = (p: string): string =>
  p === "public" ? "공개" : p === "private" ? "비공개" : "요청필요";

const pjtTitle = (id: string): string =>
  DATA.projects.find((p) => p.id === id)?.title || "프로젝트";

/* ===== F-MA 대시보드: AI 추천 과제 / 차트 헬퍼 ===== */
function recFilterChips(): string {
  const tabs: [string, string][] = [
    ["all", "전체"],
    ["모집중", "모집중"],
    ["진행중", "진행중"],
  ...
  return tabs
    .map(
      ([k, label]) =>
        `<button class="chip sm ${recFilter === k ? "active" : ""}" data-rf="${k}">${label}</button>`
    )
    .join("");
}
// 추천 카드(찜·클릭→매칭상세) 바인딩 — 필터 리렌더 시 재호출
function bindRecCards(): void {
  bindFavButtons();
  document.querySelectorAll<HTMLElement>("#recTrack .rec-card").forEach((c) => {
    c.addEventListener("click", (e) => {
      if ((e.target as HTMLElement).closest(".fav-btn")) return;
      matchView = "student";
      openMatchDetail(c.dataset.pjt as string, myTalent().name);
    });
  });
}
function recommendCards(): string {
  ...
  return recs
    ...
    <div class="rec-card" data-pjt="${p.id}">
      <div class="rec-top">
        ...
        <button class="fav-btn ${fav ? "on" : ""}" data-pjt="${p.id}" title="관심 과제">${
        fav ? "★" : "☆"
      }</button>
      </div>
      ...
      <div class="tags">${p.category
        ...
        .map((c) => `<span class="tag">#${c}</span>`)
        .join("")}</div>
      <div class="rec-foot muted sm">👥 ${p.members.length}명 · <span class="badge ${stClass(
        p.status
      )}">${p.status}</span></div>
    </div>`;
    })
...
const SKILL_AXES: string[] = DATA.skills.map((s) => s.label);

// 현재 로그인 사용자의 인재 프로필 (역량 6축·태그)
function myTalent(): Talent {
  return DATA.talents.find((t) => t.name === DATA.user.name) || DATA.talents[0];
}

// 요구역량 충족률 % : Σ min(보유, 요구) / Σ 요구
function skillMatch(req: Record<string, number>, skills: Record<string, number>): number {
  let need = 0,
    have = 0;
...
function matchedTags(reqTags: string[], talentTags: string[]): string[] {
...
// 부족 역량 (요구 > 보유) 만 내림차순
function gapSkills(req: Record<string, number>, skills: Record<string, number>): GapSkill[] {
...
function matchTagRow(reqTags: string[], talentTags: string[], gaps: GapSkill[]): string {
...
function matchRowStudent(p: Project): string {
  const me = myTalent();
...
function matchStudentView(): string {
  const me = myTalent();
  const ranked = [...DATA.projects].sort(
    (a, b) => skillMatch(b.req, me.skills) - skillMatch(a.req, me.skills)
  );
  return `
    <div class="card">
      ...
      <div class="match-list">${
        ranked.length
          ...
          : emptyState("매칭할 산학과제가 없습니다. 산학프로젝트에서 RFP를 등록하세요.", "🎯")
      }</div>
    </div>`;
}

// ----- 기업 관점: 선택한 RFP에 추천되는 인재 -----
function matchRowTalent(t: Talent, p: Project): string {
  ...
  return `
    <div class="match-row">
      <div class="mr-ring ${ringClass(pct)}"><span>${pct}<i>%</i></span></div>
      <div class="mr-body">
        <div class="mr-top"><h4>${t.name}</h4><span class="muted sm">${t.major}</span>${
    bonus ? `<span class="match-bonus" title="역량 ${base}% + 대회가점 ${bonus}">🏆 +${bonus}</span>` : ""
  }</div>
...
function jdMatchedTags(t: Talent): string[] {
...
function jdScore(t: Talent): number {
...
function matchRowTalentJd(t: Talent): string {
  const pct = jdScore(t);
  const matched = jdMatchedTags(t);
  const rfpId = DATA.projects[0]?.id || "";
  return `
    <div class="match-row">
      <div class="mr-ring ${ringClass(pct)}"><span>${pct}<i>%</i></span></div>
      <div class="mr-body">
...
function matchCompanyView(): string {
  ...
  if (!p)
    return `<div class="card">${emptyState(
      "등록된 RFP가 없습니다. 산학프로젝트에서 RFP를 등록하세요.",
      "🏢"
    )}</div>`;
  // 종합 점수 = 역량 매칭% + 대회 입상 가점 (대회 우승팀원이 상위 랭크)
  const compScore = (t: Talent): number =>
    Math.min(100, skillMatch(p.req, t.skills) + talentCompBonus(t.name));
    ...
    <div class="card">
      ...
      <div class="rfp-chips" id="rfpChips">
        ${DATA.projects
          .map(
            (x) =>
              `<button class="chip ${matchRfp === x.id ? "active" : ""}" data-rfp="${x.id}">${x.title}</button>`
          )
          .join("")}
      </div>
...
function analyzeJd(btn?: HTMLButtonElement): void {
  const ta = document.getElementById("jdInput") as HTMLTextAreaElement | null;
...
function clearJd(): void {
  jdText = "";
  render("matching");
}

// ----- 매칭 상세 모달 (레이더 오버레이 + 일치근거 + 부족역량·학습추천) -----
function openMatchDetail(rfpId: string, talentName: string): void {
...
function matchAct(rfpId: string, talent: string): void {
...
function kanbanCard(t: KanbanTask): string {
  return `
...
function kanbanColumn(col: KanbanCol, label: string, pjt: string): string {
  const cards = DATA.kanban.filter((t) => t.project === pjt && t.col === col);
  return `
  <div class="kb-col" data-col="${col}">
    <div class="kb-col-head">${label} <span class="kb-count">${
    cards.length
  }</span>${
    col === "todo"
...
function deliverableRows(pjt: string): string {
...
function providerMeta(p: Provider): { ic: string; label: string } {
  if (p === "gitlab") return { ic: "🦊", label: "GitLab 웹IDE" };
  if (p === "kaggle") return { ic: "📊", label: "Kaggle 노트북" };
  return { ic: "📓", label: "Jupyter Lab" };
}

function envCard(e: IdeEnv): string {
  ...
  <div class="card env-card">
    <div class="env-head">
      ...
      <span class="badge ${running ? "open" : "info"}">${
    running ? "● 실행중" : "○ 중지됨"
  }</span>
    </div>
    <h3 class="card-title">${e.name}</h3>
    <p class="muted sm">🧱 ${e.stack} ${e.version} · ${
    running ? `CPU ${e.cpu}% · RAM ${e.ram}%` : "리소스 0%"
  }</p>
    <div class="env-actions">
      <button class="btn ${
        running ? "primary" : "outline"
      } sm env-run-btn" data-id="${e.id}">${
    running ? "↗ 새 창 열기" : "▶ 시작"
  }</button>
      <button class="btn outline sm env-toggle-btn" data-id="${e.id}">${
    running ? "⏹ 중지" : ""
  }</button>
    </div>
  </div>`;
...
function commitRows(pjt: string): string {
  ...
  return list
    ...
    <div class="commit-row" data-id="${c.id}">
      ...
      <div class="commit-body">
        ...
        <div class="commit-meta muted sm">${c.author} · ${c.time} · <code>${
        c.id
      }</code>
          <span class="diff-stat"><b class="add">+${c.added}</b> <b class="del">−${
        c.removed
      }</b></span>
        </div>
      </div>
...
/* ===== F-CR 역량인증 / 포트폴리오 헬퍼 ===== */
function badgeCardEl(b: Badge): string {
  return `
...
function evalRows(): string {
  return DATA.evaluations
    .map(
      (e) => `
    <div class="eval-row">
      <div>
        <div class="eval-stu">🎓 ${e.student} <span class="muted sm">· ${pjtTitle(
        e.project
      )}</span></div>
...
function timelineNode(m: Milestone): string {
...
function weakRecs(job: JobTarget): string {
  // 현재 역량 대비 목표 갭이 큰 역량부터 정렬, 해당 역량 보완 추천 우선 노출
  const cur: Record<string, number> = {};
  DATA.skills.forEach((s) => (cur[s.label] = s.value));
  const gap = (skill: string): number => (job.target[skill] || 0) - (cur[skill] || 0);
  ...
  return recs
    ...
    <div class="learn-row ${g > 0 ? "weak" : ""}">
      <div>
        ...
        <div class="muted sm">보완 역량: ${r.skill}${
        g > 0 ? ` <span class="gap">(목표 대비 −${g})</span>` : ""
      }</div>
      </div>
...
function parseTags(s: string, hash: boolean): string[] {
  return s
    .split(/[,\s]+/)
    .map((x) => x.replace(/^#+/, ""))
    .filter(Boolean)
...
function addNotif(icon: string, title: string): void {
  DATA.notifs.unshift({ id: uid("N"), icon, title, time: "방금", read: false });
  renderNotifs();
}

// 빈 목록 플레이스홀더
function emptyState(msg: string, icon: string = "📭"): string {
...
function skeletonFor(page: PageKey): string {
  const cards = (n: number): string =>
    Array.from({ length: n }, skCard).join("");
  const statRow = `<div class="grid-3">${'<div class="sk-card"><div class="sk sk-stat"></div></div>'.repeat(
    3
  )}</div>`;
  ...
  return `<div class="skeleton-wrap"><div class="sk-card"><div class="sk sk-line w30"></div></div><div class="grid-2">${cards(
    4
  )}</div></div>`;
}
// 내가 등록한 항목에만 노출되는 삭제 버튼
function delBtn(kind: string, id: string): string {
  return createdItems.has(id)
    ? `<button class="del-btn" data-del="${kind}" data-id="${id}" title="삭제" aria-label="삭제">🗑</button>`
    : "";
}
// 자체 등록 항목 삭제
function deleteCreated(kind: string, id: string): void {
  if (kind === "project") {
    DATA.projects = DATA.projects.filter((p) => p.id !== id);
    hiddenProjects.delete(id); // 숨김 상태도 정리 (스테일 방지)
  } else if (kind === "team") DATA.teams = DATA.teams.filter((t) => t.id !== id);
  else if (kind === "ip") {
...
/* ----- RFP 등록 (기업) ----- */
function openRfpModal(): void {
  const axisInputs = SKILL_AXES.map(
    (k, i) =>
      `<label class="rq-axis">${k}<input class="input" id="rfpReq${i}" type="number" min="0" max="100" value="80"/></label>`
  ).join("");
  openModal(`
...
function createRfp(btn?: HTMLButtonElement): void {
  ...
  const newProject: Project = {
    ...
    members: [],
    progress: 0,
    files: [{ name: "README.md", perm: "public" as FilePerm }],
    ide: (fv("rfpIde") as "gitlab" | "kaggle") || "gitlab",
    license: fv("rfpLicense") || "MIT",
    visibility: (fv("rfpVis") as "public" | "private") || "private",
    req,
...
function createTeam(btn?: HTMLButtonElement): void {
  const name = fv("teamName");
  if (!name) {
    toast("⚠️ 팀명을 입력하세요.", "warn");
    return;
  }
  const recruiting = (document.getElementById("teamRecruit") as HTMLInputElement)?.checked ?? true;
  const id = uid("T");
  const newTeam = {
    id,
    name,
    project: fv("teamProject") || DATA.projects[0]?.title || "",
...
function applyTeam(id: string): void {
  appliedTeams.add(id);
  const t = DATA.teams.find((x) => x.id === id);
  addNotif("🙋", `팀 <b>${t?.name || ""}</b> 에 지원했습니다.`);
  toast(`✅ <b>${t?.name || "팀"}</b> 에 지원했습니다. 팀장 검토 후 알림을 드립니다.`, "success");
  render("team");
}

/* ----- IP 등록 ----- */
function openIpModal(): void {
  openModal(`
...
function createIp(btn?: HTMLButtonElement): void {
  const title = fv("ipTitle");
  if (!title) {
    toast("⚠️ 제목을 입력하세요.", "warn");
    return;
  }
  const coApply = (document.getElementById("ipCoApply") as HTMLInputElement)?.checked ?? false;
  const id = uid("IP");
  const newIp: IpItem = {
    id,
    title,
...
/* ----- 인재 제안 (기업) ----- */
function openProposeModal(name: string): void {
  openModal(`
...
function proposeTalent(name: string, btn?: HTMLButtonElement): void {
  busy(btn, () => {
    ...
    document
      .querySelectorAll<HTMLButtonElement>(`.propose-btn[data-name="${name}"]`)
      .forEach((b) => {
        b.disabled = true;
        b.textContent = "✓ 제안 완료";
      });
...
function bindPageEvents(page: PageKey): void {
  // 자체 등록 항목 삭제 버튼 (projects/team/ip 공통)
  document.querySelectorAll<HTMLElement>(".del-btn").forEach((b) => {
    b.addEventListener("click", (e) => {
      e.stopPropagation();
      deleteCreated(b.dataset.del as string, b.dataset.id as string);
    });
  ...
  /* ===== 대시보드: 기간필터 / 추천 슬라이드 / 찜 ===== */
  if (page === "dashboard") {
    const sel = document.getElementById("periodSel") as HTMLSelectElement | null;
    if (sel)
      sel.addEventListener("change", () => {
        dashPeriod = sel.value as Period;
        const bars = document.getElementById("eduBars");
        if (bars) bars.innerHTML = eduBars(dashPeriod);
        const leg = document.getElementById("donutLegend");
        if (leg) leg.innerHTML = donutLegend(dashPeriod);
        drawDemandDonut(dashPeriod);
      });
    const track = document.getElementById("recTrack");
    document.querySelectorAll<HTMLElement>(".rec-arrow").forEach((b) => {
    ...
    bindRecCards();
    // 추천 과제 모집상태 필터
    document.querySelectorAll<HTMLElement>("#recFilters .chip").forEach((chip) => {
      chip.addEventListener("click", () => {
        recFilter = chip.dataset.rf as string;
        document
  ...
  if (page === "matching") {
    document.querySelectorAll<HTMLElement>("#matchSeg .seg-btn").forEach((b) => {
      b.addEventListener("click", () => {
        matchView = b.dataset.view as "student" | "company";
        render("matching");
      });
    });
    document.querySelectorAll<HTMLElement>("#rfpChips .chip").forEach((c) => {
      c.addEventListener("click", () => {
        matchRfp = c.dataset.rfp as string;
        render("matching");
      });
    });
    document.querySelectorAll<HTMLElement>(".mr-detail").forEach((b) => {
      b.addEventListener("click", () =>
        openMatchDetail(b.dataset.rfp as string, b.dataset.talent as string)
      );
    });
  }
  if (page === "talent") {
    const r = document.getElementById("codeRange") as HTMLInputElement;
    const v = document.getElementById("codeVal") as HTMLElement;
    const s = document.getElementById("talentSearch") as HTMLInputElement;
    const list = document.getElementById("talentList") as HTMLElement;
    const refresh = (): void => {
      const min = +r.value;
    ...
    s.oninput = refresh;
    const blind = document.getElementById("blindToggle") as HTMLInputElement | null;
    if (blind)
    ...
    list.addEventListener("click", (e) => {
      ...
      const fav = target.closest(".talent-fav") as HTMLElement | null;
      if (fav) {
        toggleFavTalent(fav.dataset.fav as string);
        return;
      }
      const btn = target.closest(".propose-btn") as HTMLButtonElement | null;
      if (btn && !btn.disabled) openProposeModal(btn.dataset.name as string);
    });
  }
  /* ===== 산학프로젝트: 파일 열람권한 ===== */
  if (page === "projects") {
    ...
    if (reqBtn) reqBtn.addEventListener("click", openIncomingModal);

    // RFP 등록
    document.getElementById("rfpRegBtn")?.addEventListener("click", openRfpModal);

    // 검색어 → 그리드 필터 (리렌더 후 카드 재바인딩)
    const ps = document.getElementById("projectSearch") as HTMLInputElement | null;
    if (ps)
      ps.oninput = () => {
    ...
    // 상태 필터 탭
    document
      .querySelectorAll<HTMLElement>("#projectFilterTabs .cat-tab")
      .forEach((tab) => {
        tab.addEventListener("click", () => {
          projectFilter = tab.dataset.pf as string;
          document
  ...
  /* ===== 팀구성: 팀 만들기 / 지원하기 ===== */
  if (page === "team") {
    document.getElementById("teamNewBtn")?.addEventListener("click", openTeamModal);
    document.querySelectorAll<HTMLElement>(".apply-team").forEach((b) => {
      b.addEventListener("click", () => applyTeam(b.dataset.team as string));
    });
  }
  /* ===== 지식재산권: 등록 / 검색 / 필터 / 북마크 / 상세 ===== */
  if (page === "ip") {
    document.getElementById("ipRegBtn")?.addEventListener("click", openIpModal);
    bindIpCards();
    const is = document.getElementById("ipSearch") as HTMLInputElement | null;
    if (is)
      is.oninput = () => {
        ipQuery = is.value;
        refreshIpGrid();
    ...
    document
      .querySelectorAll<HTMLElement>("#ipFilterTabs .cat-tab")
      .forEach((tab) => {
        tab.addEventListener("click", () => {
          ipFilter = tab.dataset.ipf as string;
          document
    ...
    // 북마크만 보기 토글
    document.getElementById("ipBookmarkBtn")?.addEventListener("click", () => {
      if (!ipBookmarks.size) {
  ...
  /* ===== 관리자: 회원 검색 / 구분 필터 ===== */
  if (page === "admin") {
    const ms = document.getElementById("memberSearch") as HTMLInputElement | null;
    if (ms)
      ms.oninput = () => {
    ...
    document
      .querySelectorAll<HTMLElement>("#memberRoleTabs .cat-tab")
      .forEach((tab) => {
        tab.addEventListener("click", () => {
          memberRole = tab.dataset.mr as string;
          document
  ...
  if (page === "infra") {
    // 카테고리(전체/장비/공간) 탭
    document
      .querySelectorAll<HTMLElement>("#infraCatTabs .cat-tab")
      .forEach((tab) => {
        tab.addEventListener("click", () => {
          infraCat = tab.dataset.cat as InfraCat;
          document
            .querySelectorAll("#infraCatTabs .cat-tab")
            .forEach((t) => t.classList.remove("active"));
          tab.classList.add("active");
          const list = document.getElementById("infraList");
          if (list) list.innerHTML = infraCards();
    ...
    if (myBtn) myBtn.addEventListener("click", openMyReservationsModal);
  }
  ...
  if (page === "workspace") {
    document.querySelectorAll<HTMLElement>(".ws-tab").forEach((tab) => {
      tab.addEventListener("click", () => {
        currentWorkspace = tab.dataset.pjt as string;
        render("workspace");
      });
    });
    bindKanbanDnD();
    document.querySelectorAll<HTMLElement>(".kb-add").forEach((b) => {
      b.addEventListener("click", () => openAddTaskModal(b.dataset.pjt as string));
    });
    const comp = document.querySelector<HTMLElement>(".ws-complete-btn");
    if (comp && !(comp as HTMLButtonElement).disabled)
      comp.addEventListener("click", () =>
        completeProject(comp.dataset.pjt as string)
      );
    // 결과물 업로드(드롭존)
    const drop = document.getElementById("deliverDrop");
    if (drop) {
      const doUpload = (): void => uploadDeliverable(currentWorkspace);
  ...
  if (page === "ide") {
    ...
    if (nb) nb.addEventListener("click", openNewEnvModal);
    document.querySelectorAll<HTMLElement>(".env-run-btn").forEach((b) => {
      b.addEventListener("click", () => runEnv(b.dataset.id as string));
    });
    document.querySelectorAll<HTMLElement>(".env-toggle-btn").forEach((b) => {
      b.addEventListener("click", () => toggleEnv(b.dataset.id as string));
    });
    // GitLab 저장소 서브탭(커밋/브랜치/MR/이슈/파이프라인) + 패널 + 브랜치 선택
    bindRepoTabs();
    bindRepoPanel();
    const bs = document.getElementById("repoBranchSel") as HTMLSelectElement | null;
    if (bs)
      bs.addEventListener("change", () =>
        toast(`🌿 브랜치 <b>${bs.value}</b> 로 전환했습니다.`, "info")
      );
  }
  ...
  if (page === "portfolio") {
    // 배지 카테고리 필터
    document
      .querySelectorAll<HTMLElement>("#badgeCats .cat-tab")
      .forEach((tab) => {
        tab.addEventListener("click", () => {
          document
            ...
            .forEach((t) => t.classList.remove("active"));
          tab.classList.add("active");
          const cat = tab.dataset.cat as string;
    ...
    bindBadgeClicks();
    // 평가하기
    document.querySelectorAll<HTMLElement>(".eval-btn").forEach((b) => {
      b.addEventListener("click", () => openEvalModal(b.dataset.id as string));
    });
    // 포트폴리오 내보내기 / 공유
    const ex = document.getElementById("pfExportBtn");
    if (ex)
      ex.addEventListener("click", () =>
        toast("📥 디지털 포트폴리오를 PDF로 내보냈습니다. (다운로드 시작)", "success")
      );
    const sh = document.getElementById("pfShareBtn");
    if (sh)
      sh.addEventListener("click", () =>
        toast("🔗 공유 링크가 클립보드에 복사되었습니다.", "info")
      );
  }

  /* ===== 진로 로드맵 / 학습 가이드 ===== */
  if (page === "roadmap") {
    const sel = document.getElementById("jobSelect") as HTMLSelectElement | null;
    if (sel)
      sel.addEventListener("change", () => {
...
function bindBadgeClicks(): void {
  document
    .querySelectorAll<HTMLElement>(".badge-item.clickable")
    .forEach((el) => {
      el.addEventListener("click", () =>
        openBadgeDetail(el.dataset.badge as string)
      );
    });
}

function bindInfraBookButtons(): void {
  document.querySelectorAll<HTMLElement>(".infra-book-btn").forEach((btn) => {
    btn.addEventListener("click", () =>
      openInfraReservation(btn.dataset.id as string)
    );
  });
}
...
function loadState(): void {
  try {
    if (localStorage.getItem("studinno-data-v") !== DATA_VERSION) return;
    const saved = localStorage.getItem("studinno-data");
    if (saved) {
      const obj = JSON.parse(saved) as Record<string, unknown>;
      Object.keys(obj).forEach((k) => {
        (DATA as unknown as Record<string, unknown>)[k] = obj[k];
      });
    }
    ...
    if (ui) {
      ...
      favProjects.clear();
      (u.fav || []).forEach((x: string) => favProjects.add(x));
      matchActed.clear();
      (u.acted || []).forEach((x: string) => matchActed.add(x));
      if (u.dashPeriod) dashPeriod = u.dashPeriod;
      if (u.matchView) matchView = u.matchView;
      if (u.matchRfp) matchRfp = u.matchRfp;
      talentBlind = !!u.blind;
      proposedTalents.clear();
      (u.proposed || []).forEach((x: string) => proposedTalents.add(x));
      appliedTeams.clear();
      (u.applied || []).forEach((x: string) => appliedTeams.add(x));
      createdItems.clear();
      (u.created || []).forEach((x: string) => createdItems.add(x));
      // 갭충전 신규 상태
      ipBookmarks.clear();
      (u.bookmarks || []).forEach((x: string) => ipBookmarks.add(x));
      favTalents.clear();
      (u.favTalents || []).forEach((x: string) => favTalents.add(x));
      hiddenProjects.clear();
      (u.hidden || []).forEach((x: string) => hiddenProjects.add(x));
      if (u.settings && typeof u.settings === "object") {
        Object.keys(adminSettings).forEach((k) => {
          if (typeof u.settings[k] === "boolean") adminSettings[k] = u.settings[k];
        });
      }
      if (u.projectFilter) projectFilter = u.projectFilter;
      if (u.ipFilter) ipFilter = u.ipFilter;
      if (u.recFilter) recFilter = u.recFilter;
    }
  } catch (e) {
    /* 손상된 스냅샷 무시 */
  }
...
function syncUserFromLogin(): void {
  try {
    const servers = JSON.parse(localStorage.getItem("studinno-servers") || "[]") as string[];
    const sso = JSON.parse(localStorage.getItem("studinno-sso") || "{}") as {
      gitlab?: boolean;
      kaggle?: boolean;
    };
    if (servers.length) DATA.user.servers = servers;
...
function mergeCompanySignups(): void {
  try {
    const arr = JSON.parse(
      localStorage.getItem("studinno-company-signups") || "[]"
    ) as CompanySignup[];
    arr.forEach((cs) => {
...
// 약관/개인정보 모달 (Footer)
function openDocModal(doc: string): void {
  openModal(`
...
function resetDemo(): void {
  ["studinno-data", "studinno-data-v", "studinno-ui", "studinno-company-signups"].forEach(
...
function initChrome(): void {
  document
    ...
    ?.addEventListener("click", () => goPage("dashboard"));

  const chip = document.getElementById("userChip");
  const pop = document.getElementById("profilePop");
  if (chip && pop) {
    const roleLabel =
      DATA.user.role === "student" ? "학생" : DATA.user.role === "prof" ? "교원" : DATA.user.role;
    pop.innerHTML = `
      <div class="pp-head">
    ...
    document.getElementById("ppProfile")?.addEventListener("click", () => {
      pop.hidden = true;
      goPage("portfolio");
    });
    document.getElementById("ppReset")?.addEventListener("click", resetDemo);
    document.getElementById("ppLogout")?.addEventListener("click", logout);
  }

  document.querySelectorAll<HTMLElement>(".foot-doc").forEach((a) => {
...
const modalBox = document.getElementById("modalBox") as HTMLElement;
...
function openModal(html: string): void {
  ...
  const first = modalBox.querySelector<HTMLElement>(
...
function closeModal(): void {
  ...
  lastFocused?.focus(); // 열기 전 위치로 포커스 복원
  lastFocused = null;
}
...
function busy(
  ...
  if (btn.dataset.busy === "1") return; // 이중 제출 방지
  btn.dataset.busy = "1";
...
/* ============ 열람 요청 모달 (요청자) ============ */
function openRequestModal(pjtId: string, file: string, perm: FilePerm): void {
...
function sendRequestStub(file: string): void {
  closeModal();
  toast(
    `📨 <b>${file}</b> 열람 요청을 보냈습니다. 승인 시 알림을 드립니다.`,
    "info"
  );
}

/* ============ 권한 수정 모달 (소유자/학생) ============ */
function openPermEditModal(pjtId: string): void {
      ...
      ${pjt.files
        ...
        <div class="perm-edit-row">
          ...
          <select class="perm-select" data-idx="${i}">
            <option value="public"  ${
              f.perm === "public" ? "selected" : ""
            }>🌐 공개</option>
            <option value="request" ${
              f.perm === "request" ? "selected" : ""
            }>📨 요청필요</option>
            <option value="private" ${
              f.perm === "private" ? "selected" : ""
            }>🔒 비공개</option>
          </select>
        </div>`
        )
...
function savePerm(pjtId: string): void {
  const pjt = DATA.projects.find((p) => p.id === pjtId);
  if (!pjt) return;
  document.querySelectorAll<HTMLSelectElement>(".perm-select").forEach((sel) => {
    pjt.files[+(sel.dataset.idx as string)].perm = sel.value as FilePerm;
  });
  closeModal();
...
/* ============ 받은 요청 처리 모달 (소유자) ============ */
function openIncomingModal(): void {
...
function handleReq(reqId: string, action: string): void {
  ...
  if (action === "approve") {
    ...
    const f = pjt?.files.find((f) => f.name === req.file);
    if (f) f.perm = "public";
  }
  // 처리된 카드 즉시 제거
...
function rerenderKanban(): void {
  ...
  document.querySelectorAll<HTMLElement>(".kb-add").forEach((b) => {
    b.addEventListener("click", () =>
      openAddTaskModal(b.dataset.pjt as string)
    );
  });
}

function bindKanbanDnD(): void {
  document.querySelectorAll<HTMLElement>(".kb-card").forEach((card) => {
    card.addEventListener("dragstart", () => {
      dragTaskId = card.dataset.id as string;
      card.classList.add("dragging");
    });
    card.addEventListener("dragend", () => card.classList.remove("dragging"));
  });
  document.querySelectorAll<HTMLElement>(".kb-drop").forEach((zone) => {
    ...
    zone.addEventListener("drop", (e) => {
      ...
      if (t) t.col = zone.dataset.col as KanbanCol;
      dragTaskId = null;
      rerenderKanban();
    });
  });
...
let kanbanSeq = 100;
function openAddTaskModal(pjt: string): void {
  const members = DATA.projects.find((p) => p.id === pjt)?.members || [];
  openModal(`
  ...
  document.getElementById("taskSave")?.addEventListener("click", () => {
    ...
    const assignee = (
      document.getElementById("taskAssignee") as HTMLSelectElement
    ).value;
    const dueRaw = (document.getElementById("taskDue") as HTMLInputElement)
      .value;
    kanbanSeq++;
    DATA.kanban.push({
      id: "K-" + kanbanSeq,
...
function completeProject(pjt: string): void {
  ...
  render("workspace");
  toast(
    `🏁 <b>${p.title}</b> 완료! 역량 평가(디지털 배지) 단계로 연계됩니다.`,
    "success"
  );
}

let deliverSeq = 100;
function uploadDeliverable(pjt: string): void {
  deliverSeq++;
  ...
  if (list) list.innerHTML = deliverableRows(pjt);
  toast("📦 산출물이 업로드되었습니다. (스토리지 연동)", "success");
}

/* ============ F-ID 클라우드 IDE / 형상관리 ============ */
let envSeq = 10;
function openNewEnvModal(): void {
  openModal(`
  ...
  `);
  document.getElementById("envCreate")?.addEventListener("click", () => {
    const name =
      (document.getElementById("envName") as HTMLInputElement).value.trim() ||
      "새 실습환경";
    const stack = (document.getElementById("envStack") as HTMLSelectElement)
      .value;
    const version =
      (document.getElementById("envVer") as HTMLInputElement).value.trim() ||
      "latest";
    const provider = (document.getElementById("envProv") as HTMLSelectElement)
      .value as Provider;
    envSeq++;
...
function runEnv(id: string): void {
  ...
  toast(
    `↗ <b>${e.name}</b> · ${providerMeta(e.provider).label}를 새 탭으로 엽니다.`,
    "info"
  );
}
...
function openDiffModal(id: string): void {
  const c = DATA.commits.find((x) => x.id === id);
  if (!c) return;
  openModal(`
    <div class="modal-head"><h3>🔍 ${c.msg}</h3>
      <button class="modal-x" onclick="closeModal()" aria-label="닫기">✕</button></div>
    <p class="modal-desc muted sm">${c.author} · ${c.time} · <code>${
    c.id
  }</code></p>
    ${c.diff
      .map(
...
/* ============ F-CR 배지 상세 / 역량 평가 ============ */
function openBadgeDetail(name: string): void {
  ...
  openModal(`
    ...
    <div class="badge-detail">
      <div class="bd-score">${b.score ?? "-"}<span>점</span></div>
      <div class="bd-info">
        <p><b>구분</b> ${b.type}</p>
        <p><b>발급</b> ${b.issuer ?? "-"} · ${b.date}</p>
        <p><b>기준</b> ${b.criteria ?? "-"}</p>
      </div>
    </div>
...
function openEvalModal(id: string): void {
  const e = DATA.evaluations.find((x) => x.id === id);
  if (!e) return;
  openModal(`
    <div class="modal-head"><h3>📝 역량 평가 — ${e.student}</h3>
      <button class="modal-x" onclick="closeModal()" aria-label="닫기">✕</button></div>
    <p class="modal-desc muted sm">${pjtTitle(e.project)} · 평가자 ${
    e.evaluator
  }</p>
  ...
  `);
  const range = document.getElementById("evalQuant") as HTMLInputElement;
  const val = document.getElementById("evalQuantVal") as HTMLElement;
  range.addEventListener("input", () => (val.textContent = range.value));
  document.getElementById("evalSubmit")?.addEventListener("click", () => {
    const quant = +range.value;
    const badge =
      (document.getElementById("evalBadge") as HTMLInputElement).value.trim() ||
      "실무역량 인증";
    const qual = (
      document.getElementById("evalQual") as HTMLTextAreaElement
    ).value.trim();
    if (!qual) {
    ...
    render("portfolio");
    toast(
      `✅ 평가 제출 완료 — <b>${e.student}</b>에게 디지털 배지를 발급했습니다. (Push)`,
      "success"
    );
  });
}
...
function searchIndex(): SearchHit[] {
  return [
    ...DATA.projects.map((p) => ({
      label: p.title,
      sub: `산학과제 · ${p.company}`,
    ...
    ...DATA.talents.map((t) => ({
      label: t.name,
      sub: `인재 · ${t.major}`,
      page: "talent" as PageKey,
      icon: "🎯",
    })),
    ...DATA.teams.map((t) => ({
      label: t.name,
      sub: `팀 · ${t.project}`,
      page: "team" as PageKey,
      icon: "👥",
    })),
    ...DATA.badges.map((b) => ({
      label: b.name,
...
function renderNotifs(): void {
  ...
  list?.querySelectorAll<HTMLElement>(".notif-item").forEach((it) => {
    it.addEventListener("click", () => {
      ...
      document.getElementById("notifScrim")?.classList.remove("show");
...
// 알림 아이콘 → 이동할 페이지
function notifPage(icon: string): PageKey {
...
function initTopbar(): void {
  ...
  const openNotif = (): void => {
    panel?.classList.add("open");
    scrim?.classList.add("show");
  };
  const closeNotif = (): void => {
  ...
  const drop = document.getElementById("globalSearchDrop") as HTMLElement | null;
  const xBtn = document.getElementById("globalSearchX") as HTMLElement | null;
  if (!wrap || !input || !drop || !xBtn) return;

  const go = (page: PageKey): void => {
    goPage(page);
    wrap.classList.remove("focus");
    input.blur();
  };
  const bindDropItems = (): void => {
    drop.querySelectorAll<HTMLElement>(".gs-item").forEach((it) => {
      it.addEventListener("mousedown", () => {
        input.value = it.dataset.q as string;
        renderDrop();
      });
    });
    drop.querySelectorAll<HTMLElement>(".gs-hit").forEach((it) => {
      it.addEventListener("mousedown", () => go(it.dataset.page as PageKey));
    });
  };
  const renderDrop = (): void => {
    ...
    if (!q) {
      ...
      drop.innerHTML =
        ...
        recentSearches
          .map(
            (t) => `<div class="gs-item" data-q="${t}"><span>🕘</span>${t}</div>`
          )
          .join("") +
  ...
  input.addEventListener("keydown", (e) => {
    ...
    if (!q) return;
    const hit = searchIndex().find((h) =>
      h.label.toLowerCase().includes(q.toLowerCase())
    );
    if (recentSearches[0] !== q) recentSearches.unshift(q);
